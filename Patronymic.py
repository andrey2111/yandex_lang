...
class Patronymer(object):
    def get_patro(self, name, feminine=False):
        ...
        if name == 'Пётр':  # Единственное имя, в отчестве воторого 'ё' нужно поменять на 'е'
            name = 'Петр'
        ...
        if match(r'(\w+)[^л]ь$', name) or match(r'(\w*)(([кхц])|([бвгджзйклмнпрстфхцчшщ]{2}))ий$', name) \
        ...
        if match(r'(\w+)[жшчщц]$', name) or match(r'(\w+)((([аэиуо])\4)|([ае]у))$', name) \
        ...
        if match(r'(\w*)(([ауоыиэяюёе][бвгджзлмнпрстфчшщ])|(нт))ий$', name):
